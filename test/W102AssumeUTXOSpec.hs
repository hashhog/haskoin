...
spec = describe "W102 AssumeUTXO snapshot loading gates" $ do
  ...
  describe "G26 netAssumeUtxo table entries" $ do
    it "mainnet has the 4 Core entries + the 944183/956407/481823 hashhog-local entries" $
      -- The four 840000/880000/910000/935000 entries are the upstream
      -- Bitcoin Core m_assumeutxo_data values (kernel/chainparams.cpp).
      -- 944183 (W163 recovery), 956407 (near-tip recovery, adf95a6) and
      -- 481823 (Track-B pre-segwit replay boundary) are hashhog-local
      -- snapshots; they are NOT Core chainparams entries.  See
      -- Consensus.hs netAssumeUtxo (mainnet).
