...
-- | Detect an uncompressed P2PK script (67 bytes: push65 + 0x04 key + Y +
-- OP_CHECKSIG).  Returns @(tag, X)@ where @tag@ is @0x04 | (Y & 1)@ — i.e.
-- the on-disk Core ScriptCompression tag — and @X@ is the 32-byte X
-- coordinate.  Matches Core's 'IsToPubKey' second branch
-- (bitcoin-core/src/compressor.cpp:47-51): the curve point must be
-- 'IsFullyValid', so we round-trip through libsecp256k1 to confirm.
detectP2PKUncompressed :: ByteString -> Maybe (Word8, ByteString)
detectP2PKUncompressed s
  | BS.length s == 67
  , BS.index s 0  == 65
  , BS.index s 1  == 0x04
  , BS.index s 66 == 0xac
  , let key65 = BS.take 65 (BS.drop 1 s)
        x     = BS.take 32 (BS.drop 2 s)
        yLast = BS.index s 65
        tag   = 0x04 .|. (yLast .&. 0x01)
        -- Re-derive the canonical uncompressed form via secp256k1 and
        -- check it round-trips byte-for-byte.  Anything else means the
        -- (X,Y) pair is not actually on the curve, and Core would not
        -- compress it (it stays in the raw passthrough path).
        compressed33 = BS.cons (0x02 .|. (yLast .&. 0x01)) x
  , Just canonical65 <- decompressPubKey compressed33
  , canonical65 == key65
  = Just (tag, x)
  | otherwise = Nothing
...
putCompressedScript script
  ...
  | Just (tag, x) <- detectP2PKCompressed script = do
      ...
      putByteString x
  | Just (tag, x) <- detectP2PKUncompressed script = do
      -- Core's ScriptCompression also compresses 65-byte uncompressed
      -- P2PK outputs (tags 0x04/0x05 = 0x04 | Y-parity).  Without this
      -- branch a haskoin-emitted snapshot would store the raw 67-byte
      -- script via VARINT(67+6), differing byte-for-byte from a Core
      -- snapshot covering the same UTXO set.
      -- Reference: bitcoin-core/src/compressor.cpp CompressScript:78-81.
      putWord8 tag        -- 0x04 or 0x05
      putByteString x
  | otherwise = do
...
getCompressedScript = do
  ...
  case nSize of
    ...
    n | n == 4 || n == 5 -> do
      -- Tags 0x04/0x05 are Core's ScriptCompression encoding of an
      -- uncompressed-P2PK output: only the X coordinate is stored, and
      -- (tag - 0x02) gives the Y parity bit of the *uncompressed* form.
      -- Reference: bitcoin-core/src/compressor.cpp DecompressScript.
      --
      -- Reconstruction:
      --   1. Build a 33-byte compressed key:
      --        prefix = 0x02 | (tag - 0x02)  (=0x02 for tag 4, 0x03 for tag 5)
      --        key33  = prefix || X[32]
      --   2. Parse + serialize uncompressed via libsecp256k1
      --      (importPubKey + exportPubKey, compression flag false).
      --   3. Wrap as P2PK: 0x41 (push 65) || key65 || 0xac (OP_CHECKSIG).
      --
      -- Fail-closed if the X coordinate is not on the curve, matching
      -- Core's behaviour (DecompressScript returns false; the snapshot
      -- load aborts).
      x <- getBytes 32
      let prefix     = 0x02 .|. (fromIntegral n - 0x02) :: Word8
          compressed = BS.cons prefix x
      case decompressPubKey compressed of
        Nothing  -> fail "getCompressedScript: invalid X coordinate \
                         \for uncompressed-P2PK (tags 0x04/0x05)"
        Just pk65 -> return $ BS.concat
          [ BS.singleton 0x41   -- OP_PUSHBYTES_65
          , pk65
